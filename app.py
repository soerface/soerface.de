...
def add_navigation() -> dict:
    endpoints = [
        (f"{bp.name}.index", bp.name.title())
        for bp in blueprints
        if bp.name not in hidden_blueprints
    ]
    items = [
        {
...
def add_last_updated() -> dict:
    ...
    """
    return {"PAGE_LAST_UPDATED": datetime.now().strftime("%c")}
...
E-Mail: me@example.com
    ...
    return render_template(
        "base.html", meta={"robots": "noindex, follow"}, raw_content=content
    )
